...
//
/////////////////////////////////////////////////////////////////////////
HRESULT CSpbRequest::Initialize(_In_ IWDFDevice *pWdfDevice, _In_ PCWSTR pszTargetPath)
{
    FuncEntry();
    ...
    if (m_fInitialized == FALSE) {
        if ((pWdfDevice == nullptr) || (pszTargetPath == nullptr)) {
            hr = E_INVALIDARG;
        } else {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr)) {
				...
				if (SUCCEEDED(hr)) {
					L2(WFN, L"m_spRemoteTarget ready now.");
				}
            }
        }
        ...
        if (SUCCEEDED(hr)) {
			L2(WFN, L"REQUEST -> m_fInitialized = TRUE");
            m_fInitialized = TRUE;
        }

		if (FAILED(hr)) {
			L2(WFN, L"m_spWdfDevice->CreateRequest failed %x", hr);
		}
    }
    ...
    _In_ SIZE_T inBufferSize)
{
    ...
    if (SUCCEEDED(hr)) {
        if ((pInBuffer == nullptr) || (inBufferSize == 0)) {
            hr = E_INVALIDARG;
        }
    }
...
#pragma warning(suppress: 6001 6101 ) // PREFast cannot understand the use of the IOCTL to write to pOutBuffer
        ...
        if (SUCCEEDED(hr)) {
			L2(WFN, L"IOCTL_SPB_EXECUTE_SEQUENCE success.");
